...
# return the highest predicted halfsite affinity of a series of DNA sequences
def get_max_halfsite_aff(seq_ser, use_ewm, temp=25, mu=9):
    seq_ser = seq_ser.copy()
    allkmer_relative_affinity = seq_ser.apply(lambda seq: calculate_relaffinity_landscape(seq, ewm=use_ewm, sequence_design=None, consensus=None, temp=temp))
    allkmer_relative_affinity = pd.DataFrame([[a[0], b[0]] for a,b in allkmer_relative_affinity.values], columns=["f.aff", "r.aff"])
    allkmer_relative_affinity["max.aff"] = allkmer_relative_affinity.apply(lambda x: max(x["f.aff"],x["r.aff"]), axis=1)

    return allkmer_relative_affinity

# return the highest predicted halfsite occupancy of a series of DNA sequences
def get_max_halfsite_occp(seq_ser, use_ewm, mu=9):
    seq_ser = seq_ser.copy()
    allkmer_relative_occupancy = seq_ser.apply(lambda seq: calcualte_occupancy_landscape(seq, ewm=use_ewm, sequence_design=None, consensus=None, mu=mu))
    allkmer_relative_occupancy = pd.DataFrame([[a[0], b[0]] for a,b in allkmer_relative_occupancy.values], columns=["f.occp", "r.occp"])
    allkmer_relative_occupancy["max.occp"] = allkmer_relative_occupancy.apply(lambda x: max(x["f.occp"],x["r.occp"]), axis=1)

    return allkmer_relative_occupancy